                  ...
                  dataLineGroup
                    .call((g) => {
                      g.select("g.chart-data-draw-line-circle").call((g) => {
                        ...
                        circleG.each((line, i) => {
                          circleG
                            .selectAll(`circle.child-circle-child-${i}`)
                            .data(line.data)
                            .join("circle")
                            .classed(`child-circle-child-${i}`, true)
                            .style("pointer-events", "none")
                            .attr("pointer-events", "none")
                            .attr("cx", (d) => xScale(line.getX(d)))
                            .attr("cy", (d) => {
                      ...
                      return move$.pipe(
                        tap((evt) => {
                          ...
                          const _tooltipData = lines.map((line) => {
                            const bisectDate = bisector<T, Date>((d, t) => {
                              const r = line.getX(d);
                          ...
                          tooltipGroup
                            ...
                            .call((g) => {
                              ...
                              g.selectAll(`circle.line-chart-tip-circle-left`)
                                ...
                                .style("fill", "#ffffff")
                                .attr("cy", (cfg) => {
                                  const curTooltipData = cfg.data.find((d) =>
                                    filterTooltip.some((item) =>
                                      isEqual(cfg.getX(d), cfg.getX(item))
                                    )
                                  );
                                  if (!curTooltipData) {
                                    return Infinity;
                                  }
                                  const datum = cfg.getY(curTooltipData);
                                  if (datum == null) {
                                    return Infinity;
                                  }
                                  return yScale(datum as number);
                                });
                            })
                            .call((g) => {
                              g.attr("opacity", 1);
                          ...
                          tooltip
                            ...
                            .call(() => {
                              const formatLinesConfig = lines
                                .filter((ev) =>
                                  ev.data.filter((leftD) =>
                                    filterTooltip.some(
                                      (rightD) =>
                                        isEqual(
                                          ev.getX(leftD),
                                          ev.getX(rightD)
                                        ) && ev.getY(leftD) === ev.getY(rightD)
                                    )
                                  )
                                )
                                .map((ev) => ({
                                  ...ev,
                                  data: ev.data.find((leftD) =>
                                    filterTooltip.some(
                                      (rightD) =>
                                        isEqual(
                                          ev.getX(leftD),
                              ...
                              tooltipBoxDatumInfo
                                .style(
                                  "grid-template-rows",
                                  `repeat(${
                                    formatLinesConfig.length > 5
                                      ? 5
                                      : formatLinesConfig.length
                                  }, minmax(0, 1fr))`
                                )
                                .call((div) => {
                                  div
                                    .selectAll(`div.datum-info-item`)
                                    .data(formatLinesConfig)
                                    .join("div")
      ...
      <div className="flex justify-start items-center flex-wrap">
        ...
        {lineConfigs.map((d, i) => (
          <CheckBox
            ...
            label={d.label ?? "-"}
            labelColor={d.color}
            checked={!disabledState[d.uniqueId as string] ?? false}
            onChange={(e) => {
              setDisabledLines({
                ...disabledState,
